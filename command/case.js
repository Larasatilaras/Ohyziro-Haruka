...
case 'infobot':
if (!isHaruka) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
			reply('Update bot selanjutnya silahkan tanya kepada own ><')
break
case 'owner':{
	if (!isHaruka) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
		...
		for (let i of ownerContact.map(v => v + '@s.whatsapp.net')) {
			...
			ini_list.push({
				...
				hehe = await haruka.sendMessage(from, {
					...
					haruka.sendMessage(from, `Haii Kak ${pushname} 👋🏻, Nih Contact Owner Ku, Mau Tau Soal Apa Ya Kak😣`, text, {quoted: hehe})
				}
			break
...
if (!isHaruka) return sendButMessage(from, lang.noregis(pushname), `Klik Button Untuk Verify`, [{buttonId: '.daftar',buttonText: {displayText: `Daftar`,},type: 1,}], {quoted: fgif});
    if (args.length == 0) return reply(`Example: ${prefix + command} Ohyziro`)
    ini_txt = args.join(" ")
