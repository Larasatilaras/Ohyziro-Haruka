...
const starts = async (haruka = new WAConnection()) => {
	...
	CFonts.say(`Bot WhatsApp Created By ZeeoneOfc & Ohyziro`, {
		font: 'console',
		align: 'center',
	...
	haruka.on('group-participants-update', async (anu) => {
		...
		try {
						...
						if (anu.action == 'add' && !num.includes(haruka.user.jid)) {
							...
							try{
							await sendButLoc(mdata.id, `Welcome @${num.split('@')[0]} to ${mdata.subject}` + '\n' + lang.welcome(), `Welcome Message By ${ownername}`,welcome, [{"buttonId": `yyy`,"buttonText": {"displayText": "Welcome 👋🏻"},"type": "RESPONSE"}], {contextInfo: { mentionedJid: [num]}})
							} catch {
								await sendButLoc(mdata.id, `Welcome @${num.split('@')[0]} to ${mdata.subject}` + '\n' + lang.welcome(), `Welcome Message By ${ownername}`,ppmem2, [{"buttonId": `yyy`,"buttonText": {"displayText": "Welcome 👋🏻"},"type": "RESPONSE"}], {contextInfo: { mentionedJid: [num]}})
							}
							...
							try{
							await sendButLoc(mdata.id, `Goodbye @${num.split('@')[0]}\n▢ Leave from group:\n${mdata.subject}` + '\n' + lang.leave(), `Leave Message By ${ownername}`,goodbye, [{"buttonId": `yyy`,"buttonText": {"displayText": "Sayonara 👋🏻"},"type": "RESPONSE"}], {contextInfo: { mentionedJid: [num]}})
							} catch {
								await sendButLoc(mdata.id, `Goodbye @${num.split('@')[0]}\n▢ Leave from group:\n${mdata.subject}` + '\n' + lang.leave(), `Leave Message By ${ownername}`,ppmem2, [{"buttonId": `yyy`,"buttonText": {"displayText": "Sayonara 👋🏻"},"type": "RESPONSE"}], {contextInfo: { mentionedJid: [num]}})
							}
